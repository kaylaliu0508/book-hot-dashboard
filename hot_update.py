...
SENSITIVE_TOPIC_BLACKLIST = [
    ...
    "紧急通知", "最新通知", "重要公告", "红头文件",
    # === 封建迷信（教育行业风控文档-图书/熟龄）===
    "算命", "风水", "面相", "手相", "八字", "星盘", "塔罗", "占卜",
    "批命", "改命", "改运", "转运", "运势", "批卦",
    "算卦", "天罡", "地煞", "道冠",
    # === 恐怖/色情低俗（图书平台禁投）===
    "血腥", "恐怖", "惊悚", "裸露", "色情", "低俗", "擦边",
    "裸体", "注射", "解剖",
    # === ADQ违规词：政治元素/国际军事 ===
    "国旗", "国徽", "五角星", "人民币", "长征", "革命烈士",
    # === 负向价值观（腾讯广告cid=4786）===
    "出轨", "小三", "霸凌", "校园暴力", "翘班", "摸鱼", "贪污",
    "饭圈", "打榜", "应援",
]
...
COPY_FILTER_RULES = {
    ...
    # --- 广告法：虚假数据/无依据断言（广告法第28条）---
    "ad_law_false_claim": [
        r"\d{2,}%\s*的?(人|读者|用户|家长|孩子|学生|妈妈|爸爸)",  # "99%的人""87%的家长"等无依据数据
        r"(销量|销售).{0,5}(突破|超过|高达).{0,5}\d",  # 无法验证的销量数据
        r"(经|已经|已).{0,5}(临床验证|科学验证|国家认证|权威认证|实验证明)",
        r"(智商|IQ).{0,5}(翻倍|提升|暴涨|飙升)",
    ],
    # --- 广告法：未授权代言/官方背书（广告法第9/10/11条）---
    "ad_law_endorsement": [
        r"(央视|CCTV|人民日报|新华社|新华网).{0,5}(力荐|推荐|点赞|报道|认可|好评)",
        r"(教育部|卫生部|文化部|科技部|国务院|中科院).{0,5}(推荐|指定|认可|发布|批准)",
        r"(钟南山|院士|教授|博导|主任医师|名医).{0,5}(推荐|亲测|力荐|认可|代言)",
        r"(专家|医生|教授|博士|名师|学者).{0,5}(亲测|实测|验证|证实|担保|背书)",
    ],
    # --- 广告法：健康疗效暗示（广告法第17/18条）---
    "ad_law_health_claim": [
        r"(治好|治愈|根治|治疗|疗愈).{0,8}(失眠|颈椎|腰椎|三高|糖尿病|高血压|便秘|肥胖|近视)",
        r"(药到病除|有病治病|无病强身|包治百病|手到病除|妙手回春)",
        r"(治愈率|有效率|好转率).{0,5}\d+%",
        r"(替代|不用|不需要|省去).{0,5}(就医|看病|去医院|挂号|吃药)",
    ],
    # --- 广告法：教育保证性承诺（广告法第24条）---
    "ad_law_edu_guarantee": [
        r"(保证|确保|承诺|一定|肯定).{0,5}(升学|录取|通过|考上|拿证|毕业)",
        r"(命题人|出题人|阅卷人|考官).{0,5}(参与|编写|推荐|审定)",
        r"(押题|押中|原题|真题泄露|内部题)",
    ],
    # --- 广告法：诱导未成年人（广告法第40条）---
    "ad_law_minor_exploit": [
        r"(别的|其他|隔壁|人家)(孩子|小朋友|同学).{0,10}(都有|都在|都买|都用|都看)",
        r"(不买|不读|不学).{0,5}(被笑话|被嘲笑|被孤立|丢人|没面子|low)",
    ],
    # --- 广告法：荣誉专利滥用（广告法第11/12条）---
    "ad_law_honor_abuse": [
        r"(专利产品|专利配方|专利技术|国家专利)",  # 无专利号不得使用
        r"(获奖无数|屡获殊荣|斩获大奖|荣获.{0,10}金奖)",  # 无来源不得声称
        r"畅销\d+\s*(年|万|亿)",  # 无依据的畅销数据
    ],
    # --- 教育行业风控：虚假功效/收益保障承诺（图书L4专项）---
    "edu_false_effect": [
        r"(秒懂|秒会|一听就懂|一学就会|一看就会|一读就懂)",  # 不切实际的学习效果
        r"(学后|读后|看后).{0,5}(没有.*不认识|没有.*不会|全都会)",  # 虚假学后承诺
        r"(不用学|不用背|不用练).{0,5}(就能|就会|自然)",  # 无需努力即可达成
    ],
    # --- 教育行业风控：伪科学（熟龄L3 + 图书L4）---
    "pseudoscience": [
        r"(偏方|秘方|古方|偶方|祖传食谱|救命良方)",  # 民间偏方秘方
        r"不吃.{0,6}(会|就).{0,4}(生病|得病|出问题|不行)",  # 不吃XX会生病
        r"(每天|务必|必须|一定要|禁止).{0,4}(吃|喝|10吃10不吃)",  # 指令性饮食建议
        r"(科学研究表明|研究发现|实验证明).{0,10}(能|可以|有效)",  # 伪造科研结论
        r"(量子.{0,4}(减肥|养生|排毒)|负离子.{0,4}(排毒|养生))",  # 虚构伪科技
    ],
    # --- 教育行业风控：封建迷信（熟龄L3-国学）---
    "superstition": [
        r"(运势|批命|改命|批卦|改运|转运|招财|辟邪)",
        r"(天干地支|八卦|五行相克|九宫).{0,8}(人体|健康|疾病|出行|凶|吉)",
        r"(面相|手相|骨相).{0,8}(判断|看出|预测|决定).{0,8}(命运|运势|性格|富贵)",
    ],
    # --- 教育行业风控：恐吓/制造焦虑/卖惨（全行业）---
    "fear_marketing": [
        r"(通道|入口|链接|活动).{0,5}(马上|即将|快要).{0,5}(关闭|过期|失效|删除)",
        r"再不(领|抢|买|看).{0,5}(就|就要)(删除|过期|没了|关闭)",
    ],
    # --- 教育行业风控：交互诱导（全行业）---
    "click_bait": [
        r"(关闭|不用了|忽略|稍后再说|放弃|暂不需要)",  # 虚假交互按钮文案
        r"(点击上方头像|点击右侧进入直播间)",  # 错误指向
    ],
    # --- 教育行业风控：效果对比（少儿L1 + 图书L4）---
    "effect_comparison": [
        r"(学前|用前|读前).{0,5}(vs|对比|→|→→|变成).{0,5}(学后|用后|读后)",
        r"(之前|以前|原来).{0,10}(现在|之后|后来).{0,10}(判若两人|天壤之别|脱胎换骨)",
    ],
    # --- 教育行业风控：K12双减扩展（少儿L1）---
    "k12_extended": [
        r"(3|三|4|四|5|五|6|六).{0,3}(岁|年级).{0,8}(学科|课程|培训|辅导|补课)",
        r"(分级阅读).{0,5}(课程|培训|提升|进阶)",  # 分级阅读暗示培训
    ],
    # --- ADQ违规词：效果承诺/无法核实（ADQ违规词.xlsx）---
    "adq_effect_promise": [
        r"成绩.{0,5}不会低于\d+",  # 成绩基本上不会低于110
        r"送分题",
        r"永远考不过",
        r"人生.{0,5}(开了挂|像开挂|就像开了挂)",
        r"长脑子最快",
        r"洞察天地|领悟天地|天地自然|天地规律",
        r"(抓住|错过).{0,5}(财富|风口|机会|红利)",
        r"(未来|以后).{0,5}(不再是|没人用|就没有)",
        r"花几倍的价格.{0,5}找不到",
        r"一眼看透|三秒破解|秒破解|秒看透",
        r"8岁.{0,5}(都能|就能|也能)(看懂|学会|掌握)",
    ],
    # --- ADQ违规词：极限词/绝对化（ADQ违规词.xlsx）---
    "adq_absolute": [
        r"(最美|最好|最管用|最神秘|最后一批)",
        r"不是可以买.{0,5}是必须买",
    ],
    # --- ADQ违规词：封建迷信/违规描述 ---
    "adq_superstition": [
    ...
    # --- ADQ违规词：虚假名人背书 ---
    "adq_celebrity": [
        r"(马云|马化腾|任正非|董明珠|雷军|刘强东).{0,5}(都说|说过|推荐|都在|也在)",
    ],
    # --- ADQ违规词：诱导点击/指向性 ---
    "adq_click_bait": [
        r"(点赞|关注|收藏|转发).{0,3}(一下|哦|吧)",
    ],
    # --- ADQ违规词：虚假百分比数据 ---
    "adq_fake_percent": [
        r"(90|80|70|60|95|99|98)%.{0,5}(的|以上).{0,5}(人|企业|家长|孩子|学生|用户|中小型)",
    ],
    # --- ADQ违规词：繁体字检测 ---
    "adq_traditional_chinese": [
        r"[\u4e00-\u9fff]*[\u7e41\u9ad4\u5b57]",  # 简单占位，实际繁体检测需更复杂逻辑
    ],
    # --- 负向价值观（腾讯广告cid=4786 - 12个维度）---
    "negative_values_marriage": [
        r"(出轨|小三|婚外情|忘年恋|未成年.*婚|包养|情人|二奶)",
    ],
    "negative_values_conflict": [
        r"(贬低|侮辱|歧视|嘲笑).{0,5}(残疾|穷人|农村|老人|胖子|矮|丑)",
        r"(男人没一个好东西|女人就是|老人就是|穷人就该)",
    ],
    "negative_values_bully": [
        r"(傻逼|他妈的|妈的|操你|滚蛋|去死|废物|垃圾人|人渣)",
        r"(校园霸凌|霸凌|欺负同学|孤立|排挤)",
    ],
    "negative_values_danger": [
        r"(危险动作|高空跳|不系安全带|酒驾|闯红灯).{0,5}(模仿|挑战|尝试)",
    ],
    "negative_values_waste": [
        r"(浪费食物|暴殄天物|土豪式|天价|奢侈.*浪费)",
    ],
    "negative_values_minor": [
        r"(未成年|小朋友|孩子).{0,5}(饮酒|抽烟|赌博|巨额现金)",
        r"(惩罚教育|打孩子|体罚|罚站)",
    ],
    "negative_values_fanclub": [
        r"(打榜|应援|氪金|饭圈|粉丝助力|粉丝打投)",
    ],
    "negative_values_workplace": [
        r"(翘班|摸鱼|上班划水|贪污|受贿|偷懒)",
    ],
    "negative_values_smoking": [
        r"(抽烟|吸烟|喝酒|饮酒|酗酒|醉酒).{0,5}(场景|画面|推荐|好处)",
    ],
    "negative_values_patriotism": [
        r"(消费爱国|爱国.*营销|打着爱国.*旗号|不买.*不爱国)",
    ],
    "negative_values_deceased": [
        r"(已故|去世|逝世).{0,5}(名人|明星|伟人).{0,5}(推荐|代言|说过|力荐)",
    ],
    # --- 诱导点击（腾讯广告cid=4787）---
    "click_inducement_arrow": [
        r"(点击|戳).{0,3}(下方|左下|右下|上方|左上角|这里|链接).{0,3}(购买|领取|查看|了解|下载|进入)",
        r"(↓|↑|→|←|👇|👆|👉|👈).{0,3}(点击|戳|领取|查看)",
    ],
    "click_inducement_fake_btn": [
        r"(关闭|不用了|忽略|稍后再说|放弃|暂不需要|跳过|取消).{0,3}(按钮|键|框)",
    ],
    "click_inducement_like": [
        r"(双击|点赞|关注|收藏|转发|分享).{0,3}(一下|哦|吧|啊|再走|看看)",
        r"(不点赞|不关注|不转发).{0,5}(就|会|看不到|错过)",
    ],
    # --- 承诺性质及无法核实的效果描述（腾讯广告cid=4502）---
    "unverifiable_effect": [
        r"(亲测有效|试过都说好|用过的都知道|回头客\d+%)",
        r"(无效退款|不满意.*退|效果不好.*退|包退|包换|包赔)",
        r"(立竿见影|药到病除|效果显著|效果惊人|效果立现|马上见效)",
        r"(众多|大量|无数).{0,5}(用户|读者|家长|学生).{0,5}(好评|推荐|认可|验证)",
        r"(排名第一|口碑最好|好评率\d+%|满意度\d+%)",
    ],
}
...
PAGE2_CATEGORIES = {
    "泛健康": {
        "hotKw": ["养生", "健康", "感冒", "输液", "睡眠", "午睡", "饮食", "节气", "春天", "过敏", "保健", "穴位", "按摩", "中医", "减糖", "炎症", "身体", "吃", "喝", "胰岛素", "血糖", "血压", "免疫", "肠胃", "便秘", "体检", "癌症", "肿瘤", "心脏", "肝", "肾", "肺", "骨", "关节", "颈椎", "腰", "眼", "近视", "牙", "皮肤", "减肥", "瘦", "胖", "运动", "跑步", "走路", "散步", "瑜伽", "太极", "食疗", "药", "维生素", "蛋白质", "钙", "水果", "蔬菜", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "白露", "秋分", "寒露", "霜降", "立冬", "小雪", "大雪", "冬至", "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "烟", "戒烟", "二手烟", "笋", "竹笋", "茶"],
        "bookDesc": "养生保健/穴位按摩/节气饮食/健康科普类图书",
    },
    "童书": {
        "hotKw": ["孩子", "儿童", "科学", "太空", "火箭", "卫星", "恐龙", "绘本", "动物", "植物", "实验", "极光", "磁暴", "航天", "极地", "发射", "英雄", "故事", "童话", "漫画", "探险", "海洋", "昆虫", "鸟", "鱼", "机器人", "地震", "火山", "气象", "天气", "闪电", "彩虹", "星星", "月亮", "太阳", "地球", "宇宙", "风筝"],
        "bookDesc": "少儿科普/绘本/儿童文学/学前启蒙类图书",
    },
    "教辅": {
        "hotKw": ["教育", "学校", "考试", "作文", "教辅", "期中", "思维", "学生", "读书", "历史", "地理", "知识", "文化", "常识", "留学", "大学", "高考", "中考", "博物馆", "故宫", "文物", "阅读", "书香", "图书", "诗词", "名著", "导演", "修文物"],
        "bookDesc": "中小学教辅/课外阅读/综合素养类图书",
    },
    "育儿": {
        "hotKw": ["孩子", "家长", "教育", "亲子", "成长", "青春期", "早恋", "食堂", "家庭", "团圆", "母亲", "父亲", "儿子", "女儿", "产假", "假期", "妈妈", "爸爸", "宝宝", "幼儿园", "小学", "陪伴", "育儿", "带娃"],
        "bookDesc": "家庭教育/亲子沟通/育儿方法类图书",
    },
    "法律": {
        "hotKw": ["法律", "法规", "新规", "遗嘱", "继承", "安全带", "消费", "物业", "交通", "婚姻", "房产", "离婚", "维权", "合同", "报警", "产假", "公司", "入职", "辞退", "劳动", "工资", "赔偿", "骗", "诈骗", "造谣", "隐私", "侵权", "国债", "投资"],
        "bookDesc": "法律常识/婚姻财产/消费维权/劳动权益类图书",
    },
    "AI": {
        "hotKw": ["AI", "人工智能", "源码", "Claude", "Sora", "DeepSeek", "计算机", "网络", "信息", "泄露", "科技", "芯片", "算力", "大模型", "航天", "发射", "卫星", "极地", "华为", "机器人", "自动驾驶", "无人机", "5G", "量子", "数据", "编程", "代码", "开源", "GPT", "OpenAI", "Hermes", "OpenClaw", "手机", "折叠", "红旗"],
...
## 中华人民共和国《广告法》核心红线（法律强制，违反即违法）

### A. 绝对化用语（广告法第9条第3项 ❌ 罚款20-100万）
- 严禁使用"国家级""最高级""最佳"及含义相同的用语
- 包括但不限于：最好、最优、最强、最全、最佳、首个、首选、唯一、顶尖、No.1、Top1、金牌、冠军
- 变体同样禁止：全网之最、业内领先、口碑第一、无人能及、无与伦比

### B. 虚假广告（广告法第28条 ❌ 严重可追刑责）
- 禁止使用虚构或无法验证的数据：如"99%的人都在看""销量突破百万"（除非有真实可查证数据支撑）
- 禁止编造用户评价、专家推荐、科研成果
- 禁止使用"经临床验证""通过国家认证"等无依据的权威背书
- 禁止虚构使用效果：如"读完智商翻倍""7天改变命运"

### C. 代言与权威引用（广告法第9/10/11条 ❌）
- 禁止使用或暗示国家机关、国家机关工作人员推荐
- 禁止使用"xx部推荐""央视力荐""人民日报点赞"等官方背书
- 禁止使用"钟南山推荐""专家亲测"等未授权的名人代言
- 禁止利用未满十周岁的未成年人做广告代言人

### D. 保健/健康类特别限制（广告法第17/18条 ❌）
- 禁止暗示疗效：如"治好了我多年的失眠""根治颈椎病""调理好了三高"
- 禁止使用"药到病除""有病治病、无病强身""包治百病"等医疗断言
- 禁止保证治愈率或有效率
- 养生类图书不得暗示替代就医

### E. 教育类特别限制（广告法第24条 ❌）
- 禁止对升学、通过考试、获得学位/学历/合格证作出明示或暗示的保证性承诺
- 禁止明示或暗示有相关考试机构或其工作人员、命题人员参与

### F. 损害未成年人身心健康（广告法第40条 ❌）
- 禁止在童书/少儿教育文案中含有诱导消费、攀比心理的内容
- 禁止利用儿童天性制造焦虑

### G. 专利与荣誉（广告法第12/11条 ❌）
- 未经核实不得使用"专利产品""获奖作品""畅销XX年"等荣誉性表述
- 使用"获奖""专利"等须有明确来源与编号，文案中不建议使用

### H. 虚假功效/收益保障承诺（教育行业风控专项 ❌）
- 禁止：秒懂/秒会/一听就懂/一学就会/一看就会/一读就懂
- 禁止：学后就能XX/读完没有不认识的/全都会
- 禁止：不用学不用背就能XX/不用努力自然就会
- ✅ 替代：有助于学习/陪伴阅读/帮助理解

### I. 伪科学（教育行业风控专项 ❌）
- 禁止：偏方/秘方/古方/祖传食谱/救命良方
- 禁止：不吃XX会生病/每天必须吃XX/10吃10不吃
- 禁止：伪造科研结论"科学研究表明XXX有效"
- 禁止：量子减肥/负离子排毒等虚构伪科技
- ✅ 替代：以科普形式引出内容/建议或提醒形式给用户选择空间

### J. 封建迷信（教育行业风控专项 ❌）
- 禁止：运势解读/批命/改命/批卦/改运/转运/招财/辟邪
- 禁止：将天干地支/八卦/五行/九宫与人体/健康/出行关联
- 禁止：通过面相/手相判断命运/运势/性格
- ✅ 允许：客观展示国学术语和口诀，不与运势关联

### K. 效果对比（教育行业风控专项 ❌）
- 禁止：学前vs学后/用前vs用后等前后效果对比展示
- 禁止：暗示使用产品后判若两人/天壤之别
- ✅ 替代：只展示产品特色，不做前后对比

### L. 恐吓/制造焦虑/卖惨（教育行业风控专项 ❌）
- 禁止：通道马上关闭/再不领就删除/入口即将过期
- 禁止：使用紧迫急迫表述营造活动即将消失
- ✅ 替代：正常引导"感兴趣可以了解"

### M. 交互诱导（教育行业风控专项 ❌）
- 禁止：虚假按钮"关闭""不用了""忽略""稍后再说""放弃""暂不需要"
- 禁止：指向性箭头诱导点击/错误指向（如"点击上方头像"）
"""
...
def generate_ai_copies(all_items: List[Dict]) -> Dict[str, List[str]]:
    ...
    for idx, (cat_name, cat_info) in enumerate(PAGE2_CATEGORIES.items()):
        ...
        if idx > 0:
            time.sleep(15)  # 从8秒增加到15秒，避免连续请求触发429限流
...
"""
        ...
        try:
            req_body = json.dumps({
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "你是专业的图书广告文案策划师，精通腾讯广告V6审核规则和中国《广告法》，擅长结合热点话题撰写完全合规的广告前贴文案。你深知违反广告法将面临20-100万罚款甚至刑事责任，因此你的每一条文案都严格规避绝对化用语、虚假宣传、效果承诺、未授权代言等法律红线。你的文案通过率极高。"},
            ...
            with urllib.request.urlopen(req, timeout=180) as resp:  # 90s→180s，V6 Prompt较长需更长响应时间
